...
class BookmarkAuditor:
    ...
    def print_report(self, show_top=10, show_short=False, show_tree=False):
        ...
        print("="*70)
        print(f" Total bookmarks found        : {self.db.stats['total_bookmarks']}")
        print(f" Folders scanned              : {self.db.stats['total_folders']}")
        print(f" Unique links                 : {unique_urls}")
        print(f" Duplicates detected          : {duplicates_count} ({perc_dup:.1f}%)")
        print("="*70)
        ...
        if doublons_tries:
            print(f"\n[!] Top {show_top} most duplicated links:")
            for clean_url, occurrences in doublons_tries[:show_top]:
                ...
                for occ in occurrences:
                    ...
                    if (location, tags_info) not in paths_seen:
                        print(f"      * Folder: {location}{tags_info}")
                        paths_seen.add((location, tags_info))

    def print_tree(self):
        """
        Display the folder hierarchy exactly as read from the Firefox JSON.
        Useful to diagnose folders at the wrong nesting level.

        Usage: python bmctl.py audit -i bookmarks.json --show-tree
        """
        ...
        print("\n" + "="*70)
        print("    FOLDER TREE (Firefox traversal order)")
        print("    Verify that your subfolders have the correct parent.")
        print("="*70)
        ...
        print("="*70)
        print(f"  Total folders displayed: {len(folder_order)}")
        print()
        print("  If a folder appears without indentation but should be nested,")
        print("  it means Firefox placed it at root level. The dashboard is correct.")
        print("  Reorganize in Firefox first, then re-export.")
...
class BookmarkComparator:
    ...
    def compare(db_old: BookmarkDatabase, db_new: BookmarkDatabase, show_full=False, show_short=False):
        ...
        print("="*70)
        print(f" Unique bookmarks V1 (old)    : {len(old_urls)}")
        print(f" Unique bookmarks V2 (new)    : {len(new_urls)}")
        print(f" Net delta                    : {len(new_urls) - len(old_urls)}")
        print("="*70)
        print(f" [+] New bookmarks added      : {len(added_nodes)}")
        print(f" [-] Old bookmarks removed    : {len(removed_nodes)}")
        print("="*70)
        ...
        if added_nodes:
            title = "[+] FULL LIST OF NEW ENTRIES:" if show_full else f"\n[+] PREVIEW OF NEW ENTRIES (Max {limit}):"
            print(title)
            display_nodes = added_nodes if show_full else added_nodes[:limit]
            for node in display_nodes:
                print(f"  + {node.title[:60].ljust(60)} (Folder: {node.path})")
                print(f"    {node.original_url}")
            if not show_full and len(added_nodes) > limit:
                print(f"  ... and {len(added_nodes) - limit} more.")

        if removed_nodes:
            title = "[-] FULL LIST OF REMOVED ENTRIES:" if show_full else f"\n[-] PREVIEW OF REMOVED ENTRIES (Max {limit}):"
            print(title)
            display_nodes = removed_nodes if show_full else removed_nodes[:limit]
            for node in display_nodes:
                print(f"  - {node.title[:60].ljust(60)} (Former folder: {node.path})")
            if not show_full and len(removed_nodes) > limit:
                print(f"  ... and {len(removed_nodes) - limit} more.")
...
class BookmarkMerger:
    ...
    def merge(db_base: BookmarkDatabase, db_new: BookmarkDatabase, no_confirm: bool, output_path: str):
        ...
        print("                      MERGE SUMMARY")
        print("="*70)
        print(f" Unique bookmarks kept        : {len(final_nodes)}")
        print(f" Auto-resolved conflicts      : {stats_auto_resolved}")
        print(f" Manually resolved conflicts  : {stats_manual_resolved}")
        print("="*70)
    ...
    def _interactive_resolve(url, nodes):
        ...
        print(f"    - URL  : {url}")
        print(f"    - Title: {nodes[0].title}")
        print("    In which folder(s) do you want to keep it?")
        ...
        for i, node in enumerate(choices, 1):
            print(f"      {i}) [Keep] -> {node.path}")

        print(f"      {len(choices) + 1}) Skip / Keep most recent version")
    ...
    def _export_html(nodes, output_path):
        print(f"[*] Generating Netscape HTML: {output_path}")
...
class BookmarkDashboardGen:
    ...
    def generate(db: BookmarkDatabase, output_path: str):
        print(f"[*] Generating bmctl dashboard: {output_path}")
...
<body>
    ...
    <aside class="sidebar">
        ...
        <div class="nav-section-title">Global Views</div>
        <div style="padding: 0 12px 12px;">
            <div class="folder-item active" id="btn-all" onclick="filterByFolder('ALL')">
                <i class="fa-solid fa-globe"></i> All bookmarks
            </div>
            <div class="folder-item" id="btn-recent" onclick="filterByRecent()">
                <i class="fa-solid fa-clock"></i> Recent additions
            </div>
        </div>
    ...
    <main class="main-content">
        <header class="topbar">
            <div class="search-wrapper">
                ...
                <input type="text" class="search-input" id="search" placeholder="Global search (title, #tag, url)..." onkeyup="handleSearch()">
            </div>
            
            <div class="view-controls">
                <button class="btn-view active" onclick="switchView('dashboard', event)" title="Dashboard view"><i class="fa-solid fa-border-all"></i></button>
                <button class="btn-view" onclick="switchView('cards', event)" title="Cards view"><i class="fa-solid fa-grip"></i></button>
                <button class="btn-view" onclick="switchView('table', event)" title="Table view"><i class="fa-solid fa-list-ul"></i></button>
            </div>
        </header>
        
        <div class="workspace">
            <div class="folder-header">
                <h2 id="current-title"><i class="fa-solid fa-folder-open"></i> All bookmarks</h2>
            ...
            <div id="view-table" style="display: none;">
                <table class="table-view">
                    <thead><tr><th>Title</th><th>URL</th><th>Folder</th><th>Tags</th></tr></thead>
    ...
    window.onerror = function(msg, src, line, col, err) {{
        document.body.innerHTML = '<div style="font-family:monospace;padding:40px;color:#ff4444;background:#0a0a0a;min-height:100vh">'
            ...
            + '<p style="color:#888;margin-top:20px">Regenerate with the latest version of the script.</p>'
            + '</div>';
        return true;
    }};
        ...
        function renderData(mode, query) {{
            ...
            if (mode === 'RECENT') {{
                ...
                headerTitle = 'Recent additions (50)';
                headerIcon = 'fa-clock';
            }} else if (mode !== 'ALL') {{
            ...
            document.getElementById('current-stats').innerText = filtered.length + ' bookmark(s)';
            ...
            if (filtered.length === 0) {{
                const emptyHtml = '<div class="empty-state"><i class="fa-solid fa-ghost"></i><p>No results found.</p></div>';
                if (currentActiveView === 'dashboard') vDash.innerHTML = emptyHtml;
                if (currentActiveView === 'cards') vCards.innerHTML = emptyHtml;
                if (currentActiveView === 'table') vTable.innerHTML = '<tr><td colspan="4" style="text-align:center;padding:40px;">No data</td></tr>';
                return;
            }}
            ...
            if (currentActiveView === 'dashboard') {{
                ...
                Object.keys(widgetMap).sort((a, b) => getFolderSortKey(a) - getFolderSortKey(b)).forEach(fullPath => {{
                    ...
                    if (mode === 'ALL' || mode === 'RECENT' || query) {{
                        ...
                        const prefix = mode + ' > ';
                        if (fullPath === mode) {{
                            catTitle = '<span style="color:var(--text-muted);font-style:italic">(folder root)</span>';
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(html)
        print(f"[*] bmctl dashboard successfully generated: {output_path}")
...
class BookmarkExporter:
    ...
    def _prepare_data(db: BookmarkDatabase):
        ...
        for node in db.nodes:
            data.append({
                ...
                "Folder": node.path,
                "Tags": node.tags,
                "Date_Added": BookmarkExporter._microsecs_to_datetime(node.date_added)
            })
        return data
    ...
    def to_csv(db: BookmarkDatabase, output_path: str):
        ...
        print(f"[*] CSV export successfully generated: {output_path}")
    ...
    def to_excel(db: BookmarkDatabase, output_path: str):
        try:
            ...
            print(f"[*] Excel export successfully generated: {output_path}")
        except ImportError:
            print("[!] Error: install pandas and openpyxl.")
            ...
            with open(output_path, 'w', encoding='utf-8') as f:
                ...
                for path in sorted(folders.keys()):
                    ...
                    for node in folders[path]:
                        ...
                        date_display = f" *(added on {date_str})*" if date_str else ""
                        f.write(f"- [{title_clean}]({node.original_url}){tags_str}{date_display}\n")
                    f.write("\n")
            print(f"[*] Markdown export successfully generated: {output_path}")
...
def main():
    parser = argparse.ArgumentParser(description="Advanced toolkit for managing Firefox bookmarks.")
    subparsers = parser.add_subparsers(dest='command', required=True)

    parser_audit = subparsers.add_parser('audit', help="Audit a single bookmark export")
    ...
    parser_audit.add_argument('--show-tree', action='store_true',
        help="Display folder tree as read from JSON (debug order/hierarchy)")

    parser_compare = subparsers.add_parser('compare', help="Compare two exports")
    ...
    parser_export = subparsers.add_parser('export', help="Export to CSV/xlsx/md")
    ...
    parser_merge = subparsers.add_parser('merge', help="Merge two exports")
    ...
    parser_dashboard = subparsers.add_parser('dashboard', help="Generate an interactive HTML dashboard")
    ...
    if args.command == 'audit':
        ...
        if not os.path.isfile(args.old) or not os.path.isfile(args.new):
            print("[!] Error: both JSON files must exist.")
            return
        print("[*] Loading old export...")
        ...
        if not os.path.isfile(args.input):
            print(f"[!] Error: '{args.input}' not found.")
            return
        print(f"[*] Loading bookmarks for dashboard...")
        db = BookmarkDatabase()
        db.load_firefox_json(args.input)
